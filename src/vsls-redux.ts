 ...
 */
export function vslsStoreEnhancer(actionFilter?: (action: Action) => boolean): StoreEnhancer {
	console.warn('vslsStoreEnhancer() is deprecated. ' + 'Use createVSLSMiddleware() instead.');
...
export function createVSLSMiddleware(actionFilter?: (action: Action) => boolean): Middleware {
...
export function configureVSLSStore(options: ConfigureVSLSStoreOptions): EnhancedStore {
	const { reducer, actionFilter, ...rest } = options;
	...
	return configureStore({
		...
		middleware: (getDefaultMiddleware) => getDefaultMiddleware().concat(vslsMiddleware),
	});
}
